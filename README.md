...
脚本通过 `/etc/mihomo/mihomo-rules.sh` 自动添加 ip rule 解决：

| 优先级 | 规则 | 作用 |
|--------|------|------|
| 90 | `to 100.64.0.0/10 lookup 52` | Tailnet IP 优先走 Tailscale 路由表，避免被 Mihomo TUN 绕路 |
...
| `ip rule priority 90` | 本机访问 Tailnet IP 时优先进入 Tailscale 路由表 |

> 不再强制 `tailscaled` 进程直连。这样登录控制面和 DERP 连接可以继续走现有代理，避免在无法直连 Tailscale 控制面的网络里登录超时。
